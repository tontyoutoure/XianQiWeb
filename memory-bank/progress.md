...
- 内部进度记录：
  ...
  - 2026-03-04：按 `tdd-subagent-batch-workflow` 完成第 3 波（`M8-CT-05/06, M8-UT-04/05`）Red->Green：在 `ActionBar` 增加多种 disabled map 兼容读取，并在 `ingame-actions.ts` 新增 `createCardSelectionControllerForTest`（含 `toggleCard/clickCard/getState/getCardState`）以固定 COVER 选牌三态语义；复测 `cd frontend && npm run test -- --run tests/unit/m8-actions-stage-1-red.test.ts tests/unit/m8-actions-stage-2-red.test.ts tests/unit/m8-action-bar-stage-3-red.test.ts tests/unit/m8-selection-stage-4-red.test.ts` 通过（`16 passed`）。
  - 2026-03-04：按 `tdd-subagent-batch-workflow` 完成第 4 波（`M8-UT-06~09`）Red->Green：扩展 `createCardSelectionControllerForTest` 支持 `COVER` 选满锁定/取消回退、`PLAY` 非首位按 `legalActions.payload_cards` 收敛与取消回退，并暴露统一提交可用性状态（`canSubmit` + `submitEnabledMap/actionDisabledMap`）；复测 `cd frontend && npm run test -- --run tests/unit/m8-actions-stage-1-red.test.ts tests/unit/m8-actions-stage-2-red.test.ts tests/unit/m8-action-bar-stage-3-red.test.ts tests/unit/m8-selection-stage-4-red.test.ts tests/unit/m8-selection-stage-5-red.test.ts` 通过（`20 passed`）。
  - 2026-03-04：按 `tdd-subagent-batch-workflow` 启动新一轮并完成第 1 波（`M8-UT-10/11/12, M8-CT-07`）Red->Green：扩展 `createCardSelectionControllerForTest` 覆盖 `PLAY` 首位渐进选牌、完整组合可提交、撤销后重算可交互集合；新增 `frontend/src/components/ingame/TurnPlaysPanel.vue` 以渲染 `public_state.turn.plays` 顺序与 seat 标识；复测 `cd frontend && npm run test -- --run tests/unit/m8-actions-stage-1-red.test.ts tests/unit/m8-actions-stage-2-red.test.ts tests/unit/m8-action-bar-stage-3-red.test.ts tests/unit/m8-selection-stage-4-red.test.ts tests/unit/m8-selection-stage-5-red.test.ts tests/unit/m8-selection-stage-6-red.test.ts` 通过（`24 passed`）。
  - 2026-03-04：按 `tdd-subagent-batch-workflow` 完成新一轮第 2 波（`M8-CT-08~11`）Red->Green：新增 `frontend/src/components/ingame/PillarBoard.vue` 与 `frontend/src/components/ingame/PillarOverlay.vue`，落地公共垫牌脱敏、己方垫牌补全、主界面遮挡展示与按归属弹层全展开；复测 `cd frontend && npm run test -- --run tests/unit/m8-actions-stage-1-red.test.ts tests/unit/m8-actions-stage-2-red.test.ts tests/unit/m8-action-bar-stage-3-red.test.ts tests/unit/m8-selection-stage-4-red.test.ts tests/unit/m8-selection-stage-5-red.test.ts tests/unit/m8-selection-stage-6-red.test.ts tests/unit/m8-pillar-stage-7-red.test.ts` 通过（`28 passed`）。
