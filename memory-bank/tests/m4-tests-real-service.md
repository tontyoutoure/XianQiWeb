...
> 当前状态：`M4-API-01~14` 已完成 Red->Green（`M4-API-01~05` 结果 `5 passed, 9 skipped`；`M4-API-06~10` 结果 `5 passed, 9 deselected`；`M4-API-11~14` 结果 `4 passed, 10 deselected`；全量回归 `14 passed`）；`M4-WS-01~06` 已完成 Red（`4 failed, 2 passed`）；CC 用例仍为 `skip` 占位。
...
| M4-WS-01 ~ M4-WS-06 | 🔴 Red 已执行（部分通过） | Red 已完成 | 2026-02-21 | 按指定测试ID补齐测试体并执行 `pytest backend/tests/integration/real_service/test_m4_rs_ws_01_06_red.py -q`：结果 `4 failed, 2 passed`；通过：`WS-01/06`；失败：`WS-02~05`（均为未收到契约要求的 `GAME_PUBLIC_STATE / GAME_PRIVATE_STATE / SETTLEMENT` 事件）。 |
...
- 当前阶段约束：`M4-API-01~14` 已可执行并通过，`M4-WS-01~06` 已完成 Red；其余 real-service 用例保持 skip，占位后续按“人类指定测试ID -> 编写测试体 -> Red/Green”推进。
