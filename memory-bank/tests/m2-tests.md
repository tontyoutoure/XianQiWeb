...
| M2-UT-04 | 🔴 红阶段完成（失败符合预期） | Red | `conda run -n XQB pytest backend/tests/unit/test_m2_ut_04_08_room_registry.py -q` 失败点：`RoomRegistry.set_ready` 未实现 |
| M2-UT-05 | 🟡 红阶段意外通过 | Red（意外绿） | 同上命令下通过；当前 `join` 同房幂等契约已满足 |
| M2-UT-06 | 🟡 红阶段意外通过 | Red（意外绿） | 同上命令下通过；当前跨房自动迁移契约已满足 |
| M2-UT-07 | 🟡 红阶段意外通过 | Red（意外绿） | 同上命令下通过；当前“目标房满时迁移不丢失原成员”契约已满足 |
| M2-UT-08 | 🟡 红阶段意外通过 | Red（意外绿） | 同上命令下通过；当前 leave 冷结束重置契约已满足 |
