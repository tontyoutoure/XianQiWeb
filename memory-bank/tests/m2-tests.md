...
| M2-UT-04 | ✅ 已通过 | Red -> Green 完成 | 红阶段缺失 `set_ready`；绿阶段实现 `RoomRegistry.set_ready` 后通过 |
| M2-UT-05 | ✅ 已通过 | Red（意外绿）已确认 | 同房 join 幂等契约在红阶段已满足，回归仍通过 |
| M2-UT-06 | ✅ 已通过 | Red（意外绿）已确认 | 跨房自动迁移契约在红阶段已满足，回归仍通过 |
| M2-UT-07 | ✅ 已通过 | Red（意外绿）已确认 | 目标房满时迁移原子性在红阶段已满足，回归仍通过 |
| M2-UT-08 | ✅ 已通过 | Red（意外绿）已确认 | leave 冷结束重置契约在红阶段已满足，回归仍通过 |
...
| M2-API-06 ~ M2-API-14 | ⬜ 未开始 | 待执行 | 等待人类指定优先用例 |
