...
> 说明：当前已完成 `M3-CB-01~14`、`M3-LA-04~06` 与 LA 扩展用例（`M3-LA-13~22`）；`M3-UT-01~05` 已进入 Red，ACT 新增棋柱需求待落地测试。

| 测试ID | 当前状态 | TDD阶段 | 备注 |
|---|---|---|---|
| M3-UT-01 ~ M3-UT-05 | 🔴 失败 | Red 已执行 | 2026-02-16：已新增 `engine/tests/test_m3_red_ut_01_05.py` 并执行 `pytest engine/tests/test_m3_red_ut_01_05.py -q`；当前失败点为 `engine.core.XianqiGameEngine.init_game` 未实现 |
...
| M3-ACT-08 ~ M3-ACT-10 | 🔴 失败 | Red 已执行 | 2026-02-16：已新增 `engine/tests/test_m3_red_act_08_10_pillars.py` 并执行 `pytest engine/tests/test_m3_red_act_08_10_pillars.py -q`；当前失败点为 `engine.core.XianqiGameEngine.apply_action` 未实现 |
