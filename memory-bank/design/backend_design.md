...
- `XQWEB_SEED_CATALOG_DIR`：M8 可选；seed 台账目录路径。
- `XQWEB_SEED_ENABLE_SEED_INJECTION`：M8 可选；是否启用 REST 注入 seed 能力（`true|false`）。
...
- M8 Seed Hunter 详细行为（目录扫描、匹配、回填）见本文档 `4. M8 Seed Hunter 与 REST 注入设计`。
