...
面向：三人掀棋、账号密码登录、预设房间、实时对局、结算与“重新 ready 开下一局”。
...
### M5 结算与重新准备下一局
目标：落地 MVP 基础筹码结算和“结算后重置 ready，再次 ready 开下一局”流程。
- 实现：够棋/瓷棋/掀棋关系结算（`delta_enough / delta_reveal / delta_ceramic`）。
- 实现：`/api/games/{game_id}/settlement`。
- 实现：结算后服务端清空房间成员 ready 状态；下一局沿用 `/api/rooms/{room_id}/ready` 三人重新就绪触发开局。
- 测试/验证：
  - 结算边界用例（够/瓷/掀棋时刻够棋）通过。
  - 结算后 ready 重置与“再次 ready 开局”分支行为正确。
...
### M7 前端 MVP（非对局范围）
目标：完成“登录 -> 大厅 -> 房间”的前端闭环，并建立后续对局模块可复用的基础设施。
- 实现：登录页、大厅页、房间页（不包含对局操作区与结算详情）。
- 实现：Token 管理（含 refresh）、路由守卫、Axios 请求/响应拦截。
- 实现：`/ws/lobby` 与 `/ws/rooms/{room_id}` 的连接管理、心跳与断线重连。
- 实现：房间成员/ready/状态实时同步，以及冷结束提示（“对局结束”）。
- 实现：对局事件占位透传（可接收 `GAME_PUBLIC_STATE` / `GAME_PRIVATE_STATE` / `SETTLEMENT`，但不渲染交互）。
- 测试/验证：
  - 登录/鉴权/刷新/登出闭环走通。
  - 大厅与房间实时更新、ready/leave 主流程正确。
  - 关键异常（掉线、token 过期、服务端重启）可恢复且提示明确。

### M8 前端对局与结算 + 全链路验收与发布准备
目标：在 M7 非对局前端完成后，补齐前端对局/结算能力并完成 MVP 全链路回归。
- 前置条件：对局新增需求补齐并完成文档评审。
- 实现：对局页操作区与结算弹窗。
- 实现：按 `legal_actions` 渲染可操作按钮与 `cover_list` 选择器。
- 实现：`client_version` 冲突恢复 UI（409 后拉取最新状态并重建视图）。
- 实现：公共信息与私有信息隔离展示（不泄露他人手牌/垫牌牌面）。
- 验收：登录→加入预设房间→准备→完整对局→结算→清空 ready→再次准备→新局 全流程。
...
- 前端：Vue3 + TS 非对局页面与实时状态同步（对局/结算待定）。
...
- 前端对局交互需求待定：需求冻结前不进入 M8 对局 UI 实现与全链路验收。
