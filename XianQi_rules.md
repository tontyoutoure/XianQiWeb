...
5. 一轮游戏结束后，由棋子组合最大的玩家开始决定是否出棋/扣棋。出棋情况如前所述。如果扣棋，剩余玩家按顺序决定是否**掀棋**：若本局此前已有活跃掀棋者，则先由ta决定；否则从扣棋者的下一位（逆时针）开始。一旦一局游戏中有人开始掀棋，ta就成为活跃掀棋者。活跃掀棋者在每次有人扣棋时都可以放弃掀棋，此时ta不再成为活跃掀棋者。若当前被询问玩家选择不掀棋，则继续询问下一位；若被询问的第一位玩家选择掀棋，本次掀棋决策立即结束，游戏直接回到扣棋玩家开始出棋，不再询问第三名玩家。扣棋玩家之外若无人愿意掀棋，则游戏结束，进入结算。选择掀棋的玩家会对扣棋玩家形成**一掀**，该关系用于结算筹码。
6. 如果某位玩家在开局时不拥有任何的士/相，则称为**黑**了，本局游戏直接结束。
7. 每轮收轮并更新柱数后，若任一玩家已**瓷**（>=6柱）或两名玩家已**够**（>=3柱），本局无需继续，直接进入结算。
...
5. 如果一名玩家在已经够棋的情况下进行了掀棋，游戏结束时，如果ta没有瓷棋，没有够棋的玩家将不用向ta支付够棋筹码（扣棋筹码依然需要结算）。为便于结算，应记录掀棋当刻是否已够棋。

### 额外规则
以下的额外规则可以在游戏开始时选择启用或不启用。MVP阶段默认不启用这些额外规则。
