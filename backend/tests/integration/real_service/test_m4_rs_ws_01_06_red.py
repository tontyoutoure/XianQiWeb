"""Red-phase websocket tests against a live M4 backend service (WS 01~06)."""
...
JWT_SECRET = "m4-rs-ws-red-test-secret-key-32-bytes-minimum"
ROOM_COUNT = 3
...
def live_server(tmp_path: Path) -> Generator[tuple[str, str], None, None]:
    ...
    with run_live_server(
        tmp_path=tmp_path,
        db_filename="m4_rs_ws_01_06_red.sqlite3",
...
def test_m4_rs_ws_01_room_snapshot_without_game_only_room_update(live_server: tuple[str, str]) -> None:
    """M4-WS-01: initial room snapshot without game sends ROOM_UPDATE only."""
    base_url, ws_base_url = live_server
    _, access_token = _register_user(base_url=base_url, username="m4ws01")
...
def test_m4_rs_ws_02_room_snapshot_with_game_ordered_events(live_server: tuple[str, str]) -> None:
    """M4-WS-02: initial room snapshot with game follows ordered events."""
    base_url, ws_base_url = live_server
    context = _setup_three_players_and_start_game(base_url=base_url, username_prefix="m4ws02")
...
def test_m4_rs_ws_03_action_pushes_game_public_state(live_server: tuple[str, str]) -> None:
    """M4-WS-03: successful action pushes GAME_PUBLIC_STATE."""
    base_url, ws_base_url = live_server
    context = _setup_three_players_and_start_game(base_url=base_url, username_prefix="m4ws03")
    game_id = context["game_id"]
    ...
    async def _run() -> None:
        async with websockets.connect(
            ...
            assert public_event["payload"]["game_id"] == game_id

    asyncio.run(_run())


def test_m4_rs_ws_04_private_state_is_unicast_per_seat(live_server: tuple[str, str]) -> None:
    """M4-WS-04: GAME_PRIVATE_STATE is unicast without leaking others."""
    base_url, ws_base_url = live_server
    context = _setup_three_players_and_start_game(base_url=base_url, username_prefix="m4ws04")
    game_id = context["game_id"]
...
def test_m4_rs_ws_05_enter_settlement_pushes_settlement_event(live_server: tuple[str, str]) -> None:
    """M4-WS-05: entering settlement pushes SETTLEMENT event."""
    base_url, ws_base_url = live_server
    context = _setup_three_players_and_start_game(base_url=base_url, username_prefix="m4ws05")
    game_id = context["game_id"]
    listener_token = context["token_by_seat"][0]
    ...
    async def _run() -> None:
        async with websockets.connect(
            ...
            await _recv_until(ws, event_type="ROOM_UPDATE")
            settlement_payload = await asyncio.to_thread(
            ...
            assert settlement_event["payload"]["game_id"] == game_id

    asyncio.run(_run())


def test_m4_rs_ws_06_leave_during_playing_pushes_waiting_without_settlement(live_server: tuple[str, str]) -> None:
    """M4-WS-06: leave during playing pushes waiting room update without settlement."""
    base_url, ws_base_url = live_server
    context = _setup_three_players_and_start_game(base_url=base_url, username_prefix="m4ws06")
    listener_token = context["token_by_seat"][0]
